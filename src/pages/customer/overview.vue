...
export default {
	...
	methods: {
		...
		getList(time) {
			this.list = []
			fetchCustomerOverview(time, this.$store.state.userInfo.id).then(res => {
				if (res.data.error !== undefined) {
					return
				}
				res.data.pools.forEach(e => {
					if (e[this.$route.query.id] === undefined) {
						return
					}
					this.list.push(e[this.$route.query.id])
				})
