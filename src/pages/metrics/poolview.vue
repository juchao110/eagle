...
<div class="content">
	...
	<div class="poolview_page padding_t_80">
		...
		<mt-loadmore :top-method="loadTop" ref="loadmore" style="width:7.5rem">
			<div class="field_item" :class="{active:l.id==currentId}" v-for="l in list.racks">
...
export default {
	...
	methods: {
		...
		getList(time) {
			fetchPoolview(time, this.$route.query.owner, this.$route.query.id).then(res => {
				if (res.data.error !== undefined) {
					return
				}
				res.data.racks.sort(function(a, b) {
		...
		getSearchKey(data) {
			this.list.racks = this.copyList
			let searchResult = []
			for (let i in this.list.racks) {
				if (this.list.racks[i].name.indexOf(data) > -1 || this.list.racks[i].alias.indexOf(data) > -1) {
					searchResult.push(this.list.racks[i])
				}
			}
			this.list.racks = searchResult
		}
	}
}
</script>
