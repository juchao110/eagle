...
<div class="content">
	...
	<div class="rackview_page">
		...
		<mt-loadmore :top-method="loadTop" ref="loadmore" style="width:7.5rem">
			<div class="field_item" v-for="l in list.miners" :class="{active:l.id==currentId}">
...
export default {
	...
	methods: {
		...
		getList(time) {
			fetchRackview(time, this.$route.query.owner, this.$route.query.id, this.$route.query.rack_id).then(res => {
				...
				res.data.miners.sort(function(a, b) {
		...
		getSearchKey(data) {
			this.list.miners = this.copyList
			let searchResult = []
			for (let i in this.list.miners) {
				if (this.list.miners[i].name.indexOf(data) > -1) {
					searchResult.push(this.list.miners[i])
				}
			}
			this.list.miners = searchResult
		}
	}
}
</script>
